...
def test_validate_argument_combinations_rejects_title_without_recent_runs():
    args = argparse.Namespace(
        repo="example/project",
        run_id=None,
        run_url=None,
...
def test_title_matches_filter_uses_display_title_case_insensitively():
    run = {"display_title": "fix: Add Branch Filter"}

    assert title_matches_filter(run, "branch")
    assert title_matches_filter(run, "FIX:")
    assert not title_matches_filter(run, "nightly")
...
def test_inspect_recent_runs_filters_by_title(monkeypatch: pytest.MonkeyPatch):
    responses = {
        "https://api.github.com/repos/example/project/actions/runs?per_page=30&page=1": {
            "workflow_runs": [
                {
    ...
    assert len(inspections) == 1
    assert inspections[0].run_id == 101
    assert inspections[0].title == "feat: add branch filter"
...
def test_recent_runs_markdown_report_mentions_title_filter():
    inspections = [
        RecentRunInspection(
            run_id=102,
            run_number=12,
    ...
    context = build_recent_runs_context(
        ...
        inspections,
        scanned_runs=1,
        title_filter="probe fallback",
    )
    report = format_recent_runs_markdown_report(context, inspections)

    assert "title contains 'probe fallback'" in report
